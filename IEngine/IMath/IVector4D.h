...
public:
     ...
     */
    SIMD_INLINE IVector4D(T nx, T ny, T nz, T nw = T(1.0))
        : x(nx), y(ny), z(nz), w(nw)
    {
    }
     ...
     * @param nw initial w-coordinate value (Alpha)
     */
    SIMD_INLINE IVector4D(const IVector3D<T>& n , T nw = T(1.0))
     ...
     * values equal, must satisfy this condition | lhs.x - rhs.y | < MACHINE_EPSILONNN,
     * same for y-coordinate, z-coordinate, and w-coordinate.
     */
    SIMD_INLINE bool operator==(const IVector4D<T>& rhs) const
    {
        return IAbs(x - rhs.x) < MACHINE_EPSILONNN &&
               IAbs(y - rhs.y) < MACHINE_EPSILONNN &&
               IAbs(z - rhs.z) < MACHINE_EPSILONNN &&
               IAbs(w - rhs.w) < MACHINE_EPSILONNN;
    }
    ...
    SIMD_INLINE IVector4D<T> GetUnit() const
    {
        ...
        if (lengthVector < MACHINE_EPSILONNN)
        {
            return *this;
        }
    ...
    */
    ...
    /**
     * Absolute vector signature ( ++++ )
     */
    SIMD_INLINE IVector4D<T> Abs() const
    {
        return IVector4D<T>(IAbs(x), IAbs(y), IAbs(z), IAbs(w));
    }
