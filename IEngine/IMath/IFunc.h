...
#define MACHINE_EPSILONNN 10E-7f
...
{
    static const T  gPi = atan(1.0) * 4.0;
    return gPi;
}
template<typename T> SIMD_INLINE T ITwoPi(void)
{
    static const T  gTwoPi = atan(1.0) * 8.0;
    return gTwoPi;
}
...
template<typename T>  bool IsZero( T a, T epsilon = MACHINE_EPSILONNN )
{
...
//-------------------------------------------------------------------------------
template<typename T>  bool IIsZero( T a, T epsilon = MACHINE_EPSILONNN )
{
    return (IAbs(a) <= epsilon);
}
...
template<typename T> SIMD_INLINE bool IApproxEqual(T a, T b, T epsilon = MACHINE_EPSILONNN)
{
