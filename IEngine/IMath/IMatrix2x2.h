...
template<class T> class IMatrix2x2
{
    ...
    public:
        ...
        */
        ...
        {
            const T epsilon = MACHINE_EPSILONNN;
            for (int i = 0; i < components; ++i)
            {
        ...
        */
        ...
        SIMD_INLINE void Scale(T x, T y)
        {
        ...
        SIMD_INLINE void Scale(const IVector2D<T>& scale)
        {
            *this = *this * CreateScale(scale.x , scale.y);
        }

        SIMD_INLINE void Rotate(T angle)
        {
            *this = *this * CreateRotation(angle);
        }
