...
public:
    ...
    // Constructor
    SIMD_INLINE ITransform(const IVector3D<T>& position , const IMatrix3x3<T>& _basis = IMatrix3x3<T>::IDENTITY )
    : mPosition(position) ,
      mBasis(_basis)
    {
    ...
    // Constructor
    SIMD_INLINE ITransform(const IVector3D<T>& position , const IQuaternion<T>& _quaternion )
    : mPosition(position) ,
    ...
    // Constructor
    SIMD_INLINE ITransform(const IMatrix4x4<T>& transform)
    {
    ...
    SIMD_INLINE ITransform<T> GetInverse() const
    {
        ...
        return ITransform<T>(inv * -mPosition , inv);
    }
    ...
    {
        return ITransform<T>( (*this)(t.mPosition) , t.mBasis * mBasis);
    }


    SIMD_INLINE bool operator == (const ITransform<T>& transform2) const
    {
        return  (mPosition == transform2.mPosition) &&
