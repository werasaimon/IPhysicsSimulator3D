...
//        }
    ...
    public:
        ...
        */
        ...
        /// Initialize the quaternion using from roatate around axis to angles
        SIMD_INLINE void SetFromAxis(const IVector3D<T>& axis, T angle)
        {
            // Rotate angle axis
...
//            else
        ...
        // ...
        IVector3D<T> GetEulerAngleGimbalLock(RotSeq rotSeq = RotSeq::xyz ) const 
        {
...
//                static SIMD_INLINE IQuaternion<T> LookRotation(IVector3D<T>& lookAt, IVector3D<T>& upDirection)
//                {
...
//                }
        ...
        */
        SIMD_INLINE IQuaternion<T> operator*(const IQuaternion<T>& rhs) const
        {
            return IQuaternion(w*rhs.w - x*rhs.x - y*rhs.y - z*rhs.z,
        ...
        */
        ...
        {
            const IQuaternion<T>& lhs = *this;
            return (IAbs(lhs.w - rhs.w) < MACHINE_EPSILONNN) &&
                   (IAbs(lhs.x - rhs.x) < MACHINE_EPSILONNN) &&
                   (IAbs(lhs.y - rhs.y) < MACHINE_EPSILONNN) &&
                   (IAbs(lhs.z - rhs.z) < MACHINE_EPSILONNN);
        }
     ...
     */
     SIMD_INLINE IQuaternion<T> GetInverse() const
     {
         ...
         assert (lengthSquareQuaternion > MACHINE_EPSILONNN);
     ...
     */
     SIMD_INLINE IQuaternion<T> GetUnit() const
     {
         ...
         assert (lengthQuaternion > MACHINE_EPSILONNN);
    ...
    */
     ...
     */
     ...
     {
         IQuaternion<T> ret = FromAngleAxis(IVector3D<T>(1, 0, 0), euler_angles.x) *
                              FromAngleAxis(IVector3D<T>(0, 1, 0), euler_angles.y) *
                              FromAngleAxis(IVector3D<T>(0, 0, 1), euler_angles.z);
         return ret;
     }
    ...
    //        }
        ...
        */
         ...
         {
              return IQuaternion<T>((T(1.0) - fact) * w + fact * rhs.w, v.Lerp(fact, rhs.v));
         }
        ...
        */
        SIMD_INLINE IQuaternion<T> Slerp(T r, const IQuaternion<T>& q2) const
        {
            ...
            if (IAbs(theta) < MACHINE_EPSILONNN)
            {
                ret = *this;
            }
            else
            {
                T sinTheta = ISqrt(T(1.0) - cosTheta * cosTheta);
                if (IAbs(sinTheta) < MACHINE_EPSILONNN)
                {
                    ret.w = T(0.5) * w + T(0.5) * q2.w;
                    ret.v = GetImage().Lerp(T(0.5), q2.GetImage());
                }
                else
                {
                    T rA = ISin((T(1.0) - r) * theta) / sinTheta;
...
//                return q.FromAngleAxis(rotationAxis,T(180.0f));
//            }
...
//        }
        ...
        /// The angle between the vectors is simple to find: the dot product gives its cosine.
        /// The needed axis is also simple to find: it’s the cross product of the two vectors.
        static  SIMD_INLINE IQuaternion<T> SlerpToVector(const IVector3D<T> _start, const IVector3D<T> _dest)
        {
            IVector3D<T> start = _start.Normalized();
            IVector3D<T> dest = _dest.Normalized();

            T cosTheta = start.Dot(dest);
            IVector3D<T> rotationAxis;

//            if( (start.Cross(dest)).LengthSquare() < T(0.01) )
//            {
//                return IQuaternion<T>::IDENTITY;
//            }

            if (cosTheta < T(-1 + 0.001f))
            {
                // special case when vectors in opposite directions:
                // there is no "ideal" rotation axis
                // So guess one; any will do as long as it's perpendicular to start
                rotationAxis = IVector3D<T>::Z.Cross(start);
                if (rotationAxis.LengthSquare() < T(0.01) )
                {
                    // bad luck, they were parallel, try again!
                    rotationAxis = IVector3D<T>::X.Cross(start);
                }
                ...
                rotationAxis.Normalize();
                return FromAngleAxis(rotationAxis,T(M_PI));
            }

            rotationAxis = start.Cross(dest);

            T s = ISqrt( T((1.+cosTheta)*T(2.)) );
            T invs = T(1./s);

            return IQuaternion<T>( s * T(0.5),  rotationAxis * invs);
        ...
        SIMD_INLINE IQuaternion<T> Log() const
        {
            ...
            if(IAbs(b) <= MACHINE_EPSILONNN*IAbs(w))
            {
                if(w<0.0)
                {
